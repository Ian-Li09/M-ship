...
class I2IDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Keep instances, background plus noise
    ...
    #     bboxes = self.resize_bboxes(ann['bboxes'],scale_factor)
    #     mask = bboxes2mak(self.image_size, objects['bboxes'])
    ...
    #         bndbox_list = objects.getElementsByTagName('bndbox')
    ...
    def get_mask_from_seg(self, xml_path, scale_factor):
        ...
        for objects in objectlist:
            ...
            cv2.fillPoly(mask, [pts], 0)
        # get the mask; the region of ship fill 0&the region of the bg fill 1
        return torch.from_numpy(mask).permute(2, 0, 1)
...
if __name__ == '__main__':
    import os
    data_root = '/Users/M-ship/datasets/sargen/flist/train.flist'
    flist_dir = os.path.join(os.path.dirname(data_root.split('flist')[0]), 'flist')
    env2files = {}
    for env_name in I2IDataset.ENVS:
        path = os.path.join(flist_dir, env_name + '.txt')
        with open(path) as f:
            env2files[env_name] = {line.strip().split('/')[-1] for line in f if line.strip()}

    images = make_dataset(data_root)
    ...
    for fullpath in images:
        ...
        env = None
        for env_name, files in env2files.items():
            if fname in files:
                env = I2IDataset.ENVS.index(env_name)
                break
        if env is None:
            env = -1
        env_counter[env] += 1
        label = I2IDataset.ENVS[env] if env >= 0 else 'unknown'
        print(f'  {fname:20s}  image_env={env} ({label})')
    print()
    print(f'=== 分布 ===')
    for env_name in I2IDataset.ENVS:
        env = I2IDataset.ENVS.index(env_name)
        print(f'  {env_name} ({env}): {env_counter.get(env, 0)} 张')
    if -1 in env_counter:
        print(f'  unknown  (-1): {env_counter[-1]} 张')
#
#